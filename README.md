...
| `request-decisions` | The way to ask when work needs something only someone else can give — a person or another agent: drop every question you can answer yourself, then send one brief whose answer sheet can be replied to in a single block, each item carrying the context that answer needs. Use for any decision, sign-off, ruling, missing fact or wording you are blocked on, at brief, normal or deep depth. | [Skill](skills/request-decisions/SKILL.md) · [Brief template](skills/request-decisions/references/answer-sheet.md) · [Per-item contract](skills/request-decisions/references/item-file.md) |
...
# Choose and wire a repository's own skills, from evidence rather than description matching
