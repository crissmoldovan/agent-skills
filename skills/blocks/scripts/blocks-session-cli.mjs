...
if (command === 'create' && option('message')) {
  ...
  if (argv.includes('--wait')) result.final = await waitForBlocksFinalMessage({ finalMessageUrl: result._links.final_message.href, apiKey, timeoutMs, intervalMs });
  ...
  result = await waitForBlocksFinalMessage({ finalMessageUrl: option('final-url'), apiKey, timeoutMs, intervalMs });
} else {
  usage();
  process.exitCode = 2;
}
