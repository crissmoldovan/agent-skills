...
**Nothing was lost.** The report is printed as usual, with an added `skipped` array
naming the files, and the purge has not happened for those segments. Any tombstone
whose target still sits in one of them keeps `purged: false`, because the content is
still there. Re-run when the session finishes.

**Some of it may be half-done, and the report says which half.** A run rewrites
segments one at a time, so a skip partway through leaves earlier ones already purged.
`tombstonesMarkedPurged` lists only the tombstones this run actually flipped, and
...
run computes the same plan; only `--apply` acts on it. The one divergence is a run
that skips a segment: a dry run has nothing to skip, so it lists everything it
*would* do, while `--apply` reports what it *did*. `tombstonesMarkedPurged` names
