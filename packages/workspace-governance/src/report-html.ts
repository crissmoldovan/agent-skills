...
const kindLabel: Record<ReportNode["kind"], string> = {
  user: "User",
  domain: "Domain",
...
function renderTree(nodes: ReportNode[]): string {
  ...
  const branch = (node: ReportNode): string => {
    ...
    const pathSlug = node.label === node.slug
      ? ""
      : `<code class="node-slug">${escapeHtml(node.slug)}</code>`;
    return `<li><span class="kind">${escapeHtml(kindLabel[node.kind])}</span><strong>${escapeHtml(node.label)}</strong>${pathSlug}${
