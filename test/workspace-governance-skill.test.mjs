...
test('A10 workspace governance is portable, separately installed and root verified',async()=>{
 ...
 // The skill declared linux only while its whole read-only surface runs on macOS too; the
 // Linux-gated parts are the init/mutation commands no step here calls.
 assert.match(skill,/^platforms: \[linux, darwin\]$/m);for(const phrase of ['Linux and macOS both run the whole read-only surface','require Linux','x86_64','any architecture','Windows is not supported'])assert.ok(skill.includes(phrase),phrase);
 ...
 const pkg=JSON.parse(await readFile(join(root,'package.json'),'utf8'));assert.match(pkg.scripts.verify,/verify:governance/);assert.match(pkg.scripts['verify:governance'],/workspace-governance/);const governancePackage=JSON.parse(await readFile(join(root,'packages','workspace-governance','package.json'),'utf8'));assert.match(governancePackage.scripts.verify,/verify:effective-uid/);const readme=await readFile(join(root,'README.md'),'utf8');assert.ok(readme.includes(skill.match(/^description: (.+)$/m)[1]));assert.match(readme,/twenty-three public, portable/i);
