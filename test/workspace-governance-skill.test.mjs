...
test('A10 workspace governance is portable, separately installed and root verified',async()=>{
 ...
 // Linux-x86_64 parts are the init/mutation commands no step here calls.
 assert.match(skill,/^platforms: \[linux, darwin\]$/m);for(const phrase of ['Linux and macOS both run the whole read-only surface','Linux-x86_64 only','Windows is not supported'])assert.ok(skill.includes(phrase),phrase);
